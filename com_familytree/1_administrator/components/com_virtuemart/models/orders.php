...
// 		vmdebug('getOrdersList');
		...
		.',u.email as order_email,pm.payment_name AS payment_method'.', ft.Tree_Data as tree_data';
	...
	private function getOrdersListQuery()
	{
		return ' FROM #__virtuemart_orders as o
			...
			LEFT JOIN #__familytree_tree_data as ft
			ON o.order_number = ft.order_number AND o.order_pass = ft.order_pass
...
/*
	public function updateFamilyTree($treeData,$virtuemart_order_id){

		$db = JFactory::getDBO();
		$q = 'UPDATE `#__virtuemart_orders`SET family_tree='.$treeData.'WHERE `virtuemart_order_id`= "'.$virtuemart_order_id.'" ';
		$db->setQuery($q);
		$db->execute();
