	...
	<table class="adminlist" cellspacing="0" cellpadding="0">
		...
		if (count ($this->orderslist) > 0) {
			...
			<tr class="row<?php echo $k; ?>">
                ...
                <td><?php
                if($order->tree_data != null )
                    {
                        $text = strpos($order->tree_data, '"isFinish":true')? "Form Submitted":"Has Data";
                        echo('<a href="/index.php?option=com_familytree&view=buildtrees&orderNumber='.$order->order_number.'&orderPass='.$order->order_pass.'&isReadOnly=true"> <b>'.$text.'</b></a>');
                    }
                    ?>
			    </td>
				<?php
